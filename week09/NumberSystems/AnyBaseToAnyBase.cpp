...
unsigned fromAnyBaseToDecimal(const char* numberArr, unsigned& numberLength, unsigned fromBase) {
	unsigned result = 0;
	for (int i = numberLength - 1, multiplier = 1; i >= 0; i--, multiplier *= fromBase)
		result += charToDecimal(numberArr[i]) * multiplier;

	return result;
}

void fromAnyBaseToAnyBase(const char* fromNumber, unsigned fromNumberLength,
	char* toNumber,	unsigned& toNumberLength, unsigned fromBase, unsigned toBase) {
